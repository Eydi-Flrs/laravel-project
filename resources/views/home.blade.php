<x-home-master>
    ...
    @endsection
    @section('top-downloads')
        <div class="card my-4">
            <h5 class="card-header">Top Downloads</h5>
            <div class="card-body">
                <div class="row">
                    <div class="col-lg-12">
                        <ul class="list-unstyled mb-0">
                            @foreach($allposts->sortByDesc('downloads')->take(10) as $key=>$post)
                                <h5> {{++$key}})<a href="{{route('post',[$post->id,$post->slug])}}"> {{$post->title}} </a>:  {{$post->downloads." downloads"}}</h5>
                            @endforeach
                        </ul>
                    </div>
